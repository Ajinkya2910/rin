...
//
// The lockfile (rin.lock) captures the exact resolved state so it can
...
/// The structure of an rin.lock file
///
...
pub struct LockfileMetadata {
    /// rin version that generated this lockfile
...
pub struct LockedPackage {
    ...
    /// on `rin restore`. Mismatch → bail loudly.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub fn write(
    ...
    let content = format!(
        "# This file is auto-generated by rin. Do not edit manually.\n\
         # Commit this file to version control for reproducible installs.\n\
         # Restore with: rin restore\n\n\
         {}",
        toml_content
    );

    // Write to rin.lock in the current directory
    let path = PathBuf::from("rin.lock");
    std::fs::write(&path, content).context("Failed to write rin.lock")?;
...
pub fn read(path: impl AsRef<Path>) -> Result<Lockfile> {
    let content = std::fs::read_to_string(path.as_ref())
        .context("Failed to read rin.lock — are you in a project directory?")?;

    let lockfile: Lockfile = toml::from_str(&content).context(
        "Failed to parse rin.lock — the file may be corrupted. \
         Delete it and run `rin lock` again.",
    )?;
...
mod tests {
    ...
    fn test_lockfile_roundtrip() {
        ...
        // a stale bioc_version sends `rin install --retry` to the wrong
...
#[test]
    fn test_lockfile_github_roundtrip() {
        ...
        let path = std::env::temp_dir().join("rin-test-gh.lock");
        // Reuse write() but redirect to temp — simplest is to write+read+cleanup
        // through the real write() since it always writes to "rin.lock".
